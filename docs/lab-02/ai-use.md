...
| 9 | สร้างหน้า UI create ticket รวมถึง validation ของข้อมูลเวลา user กรอก ให้เป็นไแตาม specification.md file | ตรวจสอบความถูกต้องของหน้าเว็ป และ code ว่าเป็นไปตามที่กำหนดหรือไม่ |
| 10 | Implement Issue #11: My Tickets API based on the specification and API contract. Add requester ownership, search, filters, sorting, pagination, validation, and API tests according to the Acceptance Criteria. | ตรวจสอบโค้ด API และ Test ให้ตรงกับ api-spec.md และ Acceptance Criteria จากนั้นรัน Test และวิเคราะห์ Error ที่เกิดขึ้น โดยพบปัญหาจากการที่ Test หลายไฟล์ใช้ Database เดียวกันและทำงานพร้อมกัน จึงแก้ไขการรัน Test ด้วย --no-file-parallelism และตรวจสอบจน Test ผ่านทั้งหมด |
...
## Reflection
    ...
    ใน Issue 8 ได้มีการสร้าง API และมีการดึง API มาใช้ในหน้สเว็ป ซึ่งในตอนแรกขั้นตอนนี้เราไม่เคยทำมาก่อนทำให้ไม่รู้ว่าต้องเริ่มเขียนอะไรตรงไหน จึงใช้ AI เข้ามาและอ่านโค้ดที่ถูก Generate จนเข้าใจ รวมทั้งต้องตรวจสอบเพราะ AI ยังมี เผลอเรียก ไฟล์ .js เข้ามาใช้อีกด้วย ขั้นตอนนี้เราได้แบ่ง prompt เป็น step เหมือนเดิมเพื่อให้ง่ายต่อการตรวจสอบและทำความเข้าใจ

    ใน Issue 11 เป็นการทำ My Tickets API ซึ่งในตอนแรกเรายังไม่ค่อยเข้าใจว่าการทำ API สำหรับแสดงเฉพาะ Ticket ของ Requester แต่ละคนต้องตรวจสอบอะไรบ้าง จึงใช้ AI ช่วยอธิบายและช่วยวางโครงสร้างของ API โดยเฉพาะเรื่อง X-Requester-Id, filtering, searching, sorting และ pagination และเจอปัญหาตอนที่ run test พร้อมกันเพราะมันไปโดย test ของ lab หนึ่งที่สร้างตัวจพลองไว้ แต่ก็แก้ไขละ run test ด้วย vite