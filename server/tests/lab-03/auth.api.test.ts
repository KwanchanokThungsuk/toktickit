...
describe("Issue #18 AUTH-01 through AUTH-18", () => {
  ...
  it.each([["IT_STAFF"], ["ADMINISTRATOR"]])("requester ticket APIs reject %s users", async (role) => { const u = await makeUser({ role }); const r = await login(u.email); expect((await r.agent.get("/api/tickets")).status).toBe(403); });
